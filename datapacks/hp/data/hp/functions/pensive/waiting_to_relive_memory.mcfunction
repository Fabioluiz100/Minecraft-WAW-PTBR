...
execute as @s[tag=!cancelPensive] store success score @s tmp run title @s actionbar ["",{"text":"Aguarde até que todos interajam com o pensativo. (","color":"gold"},{"score":{"name":"@s","objective":"tmp2"},"color":"gold"},{"text":" / ","color":"gold"},{"score":{"name":"@s","objective":"tmp3"},"color":"gold"},{"text":") ","color":"gold"}]
execute as @s[tag=cancelPensive] store success score @s tmp run title @s actionbar ["",{"text":"Cancelado","color":"red"}]
